...
c.Application.logging_config = {
    'handlers': {
        'file': {
            ...
            'level': 'DEBUG',
            'filename': f'{home}/.logs/jupyter_app.log',
        }
    },
    'loggers': {
        '<application-name>': {
...
#  Default: ''
c.ServerApp.certfile = str(Path().home().joinpath(".jupyter", "zeuscert.pem"))
...
#  Default: ''
c.ServerApp.keyfile = str(Path().home().joinpath(".jupyter", "zeuskey.key"))
...
c.ServerApp.logging_config = {
    ...
    "handlers": {
        "logfile": {
            "class": "logging.FileHandler",
            "level": "DEBUG",
            "filename": f"{home}/logs/jupyter_server.log",
            "formatter": "my_format",
        },
    },
    "formatters": {
...
if 'HOSTNAME' in os.environ:
  c.PasswordIdentityProvider.hashed_password = passwd(os.environ['HOSTNAME'])
...
#  Default: ''
c.ServerApp.root_dir = str(Path().home().joinpath('workspace'))
...
#  Choices: any of ['sha384', 'sha256', 'sha3_224', 'sha1', 'sha512', 'sha224', 'sha3_384', 'md5', 'sha3_512', 'sha3_256', 'blake2b', 'blake2s']
#  Default: 'sha256'
